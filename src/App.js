  ...
  const getWeatherData = async (city) => {
    try {
      ...
      const res = await axios(
        `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${process.env.REACT_APP_KEY1}`
      );
      // console.log(res);
      let { coord } = res.data;
      const res2 = await axios(
        `https://api.openweathermap.org/data/2.5/onecall?lat=${coord.lat}&lon=${coord.lon}&units=metric&appid=${process.env.REACT_APP_KEY2}`
      );
